...
## 1b. 문제집.md 갱신 (필수 — progress.md와 항상 동기화)
파일: `문제집.md` (레포 루트). progress.md만 갱신하고 여기를 빼먹지 말 것.
1. 해당 번호가 트랙 표(DP/구현·시뮬/Bronze)에 **있으면** 그 행의 상태 칸을 `✅`로 변경.
2. 트랙 표에 **없으면**(silver 번호순 진행분 등 트랙 외 문제) `## ✅ 완료` 섹션에 한 줄 추가:
   `- <번호> <문제명> (<티어>) — <한 줄 주제>`
3. 하단 `### 메모`의 "완료/미완" 요약·날짜를 최신으로 갱신.
> 실제 진행 순서(silver 번호순)와 트랙 큐 순서가 다르므로, "다음 문제"의 기준은 progress.md다.
