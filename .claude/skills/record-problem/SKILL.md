...
## 0. 입력 확인
- 대상 백준 번호 확인(인자 또는 직전 대화에서). 풀이 파일은 `problems/<bronze|silver|gold>/<티어>_boj_<번호>.cpp`(티어 접두어 대문자 B1~G5, 예 `problems/bronze/B3_boj_2884.cpp`).
...
## 1. progress.md 갱신 (일자별 기록만)
파일: `progress.md` (레포 루트). progress.md는 **기록 표 + solved.ac 현황**만 담당(티어 큐·다음 문제는 문제집.md로 이관됨).
1. `## 기록` 표 맨 끝에 행 추가:
   `| YYYY-MM-DD | 번호 | 문제명 | 레벨 | 주제 | ✅ | 핵심 접근 + 함정/배운점 |`
> ⏳/스캐폴드(미해결) 행은 기록 표에 넣지 않는다. 통과 검증된 것만 ✅로 기록.

## 1b. 문제집.md 갱신 (필수 — 큐·다음문제의 단일 소스)
파일: `문제집.md` (레포 루트). 티어별(🥉Bronze/🥈Silver/🥇Gold) 표, 컬럼 `번호|문제|티어|트랙|주제|상태`.
1. 해당 번호 행의 **상태 칸을 `✅`**로 변경.
2. 방금 푼 행의 **`← 다음` 마커를 다음 ⬜ 문제 행으로 이동**(silver 번호순 → gold 번호순).
3. 해당 티어 헤더 완료 카운트(예: `Silver — 13/21`)와 하단 `### 메모` 요약·날짜 갱신.
...
### 속성(properties)
- `문제`(제목 — **반드시 `<백준번호>. <문제명>` 형식**, 예: `1094. 막대기`), `백준번호`(number), `유형`(JSON 배열, multi: DP·그리디·이분탐색·BFS/DFS·구현·문자열·정렬·수학·시뮬레이션 — 복수 가능), `난이도`(Bronze/Silver/Gold/Platinum), `세부난이도`(**"Silver 3"처럼 티어 풀네임+번호**, 약어 금지), `date:완료일:start`(YYYY-MM-DD), `풀이 방법`(한 줄 요약), `링크`(https://www.acmicpc.net/problem/<번호>)
...
## 💻 내가 푼 코드
<code fence cpp>problems/<bronze|silver|gold>/<티어>_boj_<번호>.cpp 의 실제 코드 그대로</code fence>
```
